  ...
  .then(response => {
    ...
    .on('data', (data) => {
      ...
      if (/[0-9\-\+]+/.test(data[7])) {
        ...
        if (!results[datum][data[7]]) results[datum][data[7]] = [0,0];
        results[datum][data[7]][data[6]=='1'? 0 : 1]++;
      }
    ...
    .on('end', () => {
      ...
      let dataByDate = preprocessed.dates.map(d => preprocessed.ageGroups.map(g => (results[d] || {})[g] || [0,0]));
      for (let i=1; i<dataByDate.length; i++) {
        for (let j in dataByDate[i]) {
          for (let k in [0,1])
          dataByDate[i][j][k] += dataByDate[i-1][j][k];
        }
      }
      preprocessed.totalsLatest=dataByDate[dataByDate.length-1];
      preprocessed.totalsPrevious=dataByDate[dataByDate.length-2];
